...
def denoise(signal : np.array, frqcy : int ) -> np.array: 
    ...
    voice_only_speak=np.array([signal[i] for i in range(len(signal)) if sum[int(i/nperseg)]>0.1 ]) # when the power is greater than 0.1, it is classified as "human speaking"
    noise_only=np.array([signal[i] for i in range(len(signal)) if sum[int(i/nperseg)]<0.1 ]) # else it is a noise
